...
//This method grabs the desired RGB values and fades to the color
void fadeToColor(uint8_t r, uint8_t g, uint8_t b, float fromBrightness)
{
    for(float j = fromBrightness; j < 1.0; j+=0.01)
    {
          setAllPixels(r, g, b, j);
          delay(5);
    }
